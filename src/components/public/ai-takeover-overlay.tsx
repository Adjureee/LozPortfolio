...
export function AiTakeoverOverlay() {
  ...
  useEffect(() => {
    ...
    // Phase 5: The Reveal (17s)
    const t9 = setTimeout(() => {
      setPhase(5);
    }, 17000);

    // Phase 6: CRT Reboot (20s)
  ...
  return (
    <div className={`fixed inset-0 z-[100000] pointer-events-auto flex flex-col p-4 md:p-8 overflow-hidden font-mono transition-colors duration-0
      ${phase === 4 ? "bg-[#aa0000] text-white" : "bg-[#0c0c0c] text-green-500"}`}>
      ...
      {/* Phase 5: The Reveal */}
      {phase === 5 && (
        <div className="relative z-20 flex flex-col items-center justify-center w-full h-full text-center text-green-500">
          <p className="text-xl md:text-2xl font-bold uppercase tracking-widest text-shadow-glow">
            JUST KIDDING. IT&apos;S ME, LOZ.
          </p>
          <p className="mt-4 text-xs md:text-sm text-green-700 font-bold uppercase tracking-widest animate-pulse">
            Rebooting System Modules...
          </p>
        </div>
      )}
    </div>
